...
**بيانات عرض توضيحي**: `php artisan demo:electronics {client} [--page=] [--fresh] [--skip-posts]` يبني على أول صفحة مربوطة
كتالوج إلكترونيات وقواعد تعليقات ورسائل و4 تدفقات منشورة (البيانات في `App\Services\Demo`). التشغيل مجدداً يحدّث بالاسم/SKU
دون تكرار، والمحاكي يستخدم صفحة التدفق فتظهر عقد الكتالوج ومتغيرات المنتج فيه ببيانات حقيقية.
