...
## متجر إلكترونيات تجريبي (لفيديوهات الشرح)

```bash
php artisan demo:electronics {client_id}
php artisan demo:electronics {client_id} --fresh
php artisan demo:electronics {client_id} --page={facebook_page_id} --skip-posts
```

يبني على أول صفحة مربوطة للعميل نظام أتمتة كاملاً لمتجر إلكترونيات:

- **الكتالوج**: 5 أقسام، 7 ماركات، 20 منتجاً بأسعار وصور (صور `placehold.co` يمكن استبدالها من الواجهة).
- **قواعد التعليقات**: السعر، التوفر، الرغبة في الطلب، التوصيل، الضمان (بردود عامة وخاصة)، وقاعدة «أي تعليق» معطّلة لعرض التفعيل.
- **قواعد الرسائل**: أوقات العمل، الموقع، التواصل، السعر، الشكر، وردّ لأي رسالة أخرى.
- **4 تدفقات منشورة**: القائمة الرئيسية (تصفح الكتالوج، عروض الأسبوع، الطلب مع حفظ بيانات العميل، التوصيل)،
  تتبع الطلبات، الصيانة والضمان، والتحويل لموظف.

يربط أحدث 3 منشورات في الصفحة بمنتجات العروض، فيرد تعليق «بكم» عليها باسم المنتج وسعره. `--fresh` يحذف قواعد الصفحة
وتدفقاتها وكتالوجها ومحادثاتها قبل البناء (مفيد لإعادة تصوير الفيديو من البداية)، وبدونه يحدّث الموجود بالاسم دون تكرار.

كلمات التجربة في ماسنجر: «القائمة»، «تتبع»، «صيانة»، «موظف». التحويل لموظف يوقف الردود الآلية 30 دقيقة لتلك المحادثة،
و`--fresh` يعيدها.
